...
cd nswitch_essential/
zip -r ../essential-latest.zip .
cd ..

if [ "$ENV" == "prod" ];then
    aws s3 cp main-latest.zip s3://nops-rules-lambda-sources/essential/essential-latest.zip --acl public-read
    aws s3 cp cloudformation_templates/essential.yml s3://nops-rules-lambda-sources/essential/essential.yml --acl public-read
else
    aws s3 cp main-latest.zip s3://nops-rules-lambda-sources/essential/essential-latest-uat.zip --acl public-read
